...
COLORS = {
    "WHITE": (220, 220, 220),       # Un bianco leggermente soft per non accecare
    "RED": (255, 80, 80),           # Rosso corallo, si sposa benissimo con le linee rosse
    "BLUE": (80, 160, 255),         # Un blu elettrico/neon ben visibile sul scuro
    "GREEN": (80, 220, 120),        # Verde menta/neon, ottimo per start e goal
    "YELLOW": (255, 220, 50),       # Giallo oro brillante
    "BLACK": (180, 180, 180)        # Lo trasformiamo in un grigio chiaro per le scritte/nodi "neri"
}
...
class Visualizer:
    def __init__(
        ...
        """Initialize the Visualizer with the network graph and the computed paths."""
        self.graph = graph
        self.master_plan = master_plan
        self.drone_colors = self._assign_drone_colors()
        
        # Calcola il turno massimo analizzando tutti i turni di arrivo nel master_plan
        self.max_turn = 0
    ...
    def _get_drone_state(self, drone_id: str, target_turn: int) -> tuple[str, str, bool]:
        """
        Interpola la posizione del drone nel turno 'target_turn' basandosi sul master_plan.
        ...
        # Se il turno cercato è oltre la fine del viaggio, il drone è arrivato (fermo)
        ...
        for i in range(len(path) - 1):
            ...
            if curr_arrival_turn <= target_turn < next_arrival_turn:
                # Se è esattamente il turno in cui arriva nella prima zona, non è in volo
                if target_turn == curr_arrival_turn:
                    return curr_zone, curr_zone, False
                # Altrimenti, sta viaggiando verso la next_zone (transito per zone RESTRICTED o BLOCKED superate)
    ...
    def run(self) -> None:
        ...
        while running:
            ...
            # 1. Disegna le Connessioni (utilizzando l'attributo .target dei Connection)
            for zone in zone_dict.values():
                ...
                for connection in zone.connections:
                    ...
                    pygame.draw.line(
                        screen, (255, 0, 0), (start_x, start_y), (end_x, end_y), 
                        connection.max_link_capacity
                    )

                    link_center_x = (start_x + end_x) / 2
                    link_center_y = (start_y + end_y) / 2
                    pygame.draw.circle(screen, (255, 255, 255), (link_center_x, link_center_y), scale / 10)
                    
                    capacity_label = font_small.render(str(connection.max_link_capacity), True, (0, 0, 0))
                    screen.blit(capacity_label, (link_center_x - 4, link_center_y - 7))
            ...
            for zone in zone_dict.values():
                ...
                pygame.draw.circle(screen, circle_color, (center_x, center_y), scale / 3)

                zone_type_letter = zone.zone_type.name[0]
                zone_type_label = font_large.render(zone_type_letter, True, (0, 0, 0), (255, 255, 255))
                drone_label = font_small.render(str(zone.max_drones), True, (0, 0, 0), (255, 255, 255))
            ...
            for drone_id in self.master_plan.keys():
                ...
                prev_loc, next_loc, in_flight = self._get_drone_state(drone_id, current_turn)
                ...
                colored_icon.fill(self.drone_colors[drone_id], special_flags=pygame.BLEND_MULT)
                screen.blit(colored_icon, (x - 15, y - 25))

            # 4. Info Testuali UI
            ui_label = font_large.render(f"Turn: {current_turn} / {self.max_turn}", True, (255, 255, 255))
